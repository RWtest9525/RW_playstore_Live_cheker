...
index b91170f6e8335976529cfd5f931133fc7f134110..d8f6fcdec3f330d1f3010105fdf803a978798d31 100644
--- a/app.py
+++ b/app.py
@@ -1,167 +1,636 @@
...
+
+    clean = url.strip()
+    # Allow direct package IDs in bulk mode (e.g. com.example.app)
+    if re.fullmatch(r"[a-zA-Z0-9_]+(\.[a-zA-Z0-9_]+)+", clean):
+        return clean
+
+    if "play.google.com" not in clean:
+        return None
+
...
+            rev_time = pytz.utc.localize(raw_time).astimezone(IST_TZ)
+        else:
+            rev_time = raw_time.astimezone(IST_TZ)
+        if rev_time.date() != target_dt:
+            continue
+
...
+
+    hint_type = st.sidebar.radio("Hint Mode", ["Show All", "No Hint (Normal .)", "Custom Symbol"], index=0)
+    custom_symbol = ""
+    if hint_type == "Custom Symbol":
+        custom_symbol = st.sidebar.text_input("Enter Symbol (e.g. # or ,,)", value="#")
+
...
+                        hints = [custom_symbol]
+                    found, fetch_errors = fetch_logic(
+                        aid,
+                        target_date,
+                        scan_depth,
...
+        st.toast("✅ Scan Complete!", icon="🎉")
+        if skipped_urls:
+            st.warning(f"Skipped invalid link/package values: {len(skipped_urls)}")
+        if total_fetch_errors:
+            st.info(
+                "Some Play Store requests failed temporarily. This can cause 0 live even when live exists. "
